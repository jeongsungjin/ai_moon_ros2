...
### STEP 0 — 하드웨어 단독 점검 (제일 먼저!)
카메라/인지 없이 모터와 서보만 확인:
```bash
ros2 launch car_planner motor_test.launch.py                                # 조향 서보 좌우 스윕 (바퀴 안 돎)
ros2 launch car_planner motor_test.launch.py mode:=throttle throttle:=0.2   # ⚠️ 차 들어올리고! 구동 모터
```
- 서보가 아예 안 움직임 → `control_node: i2c_bus` 확인 (JetRacer=1, D3-G=3),
  배선 확인: `i2cdetect -y -r 1` 출력에 `40` 이 보여야 함
- 스윕이 한쪽으로 치우침 → `servo_center_us` 조정 (기본 1500)
- 모터가 안 돎 → ESC 전원/캘리브레이션, `esc_neutral_us` 확인
- 테스트는 5초 후 자동 중립 정지 (시간 변경: `duration:=10.0`)
