...
class ShanWanGamepad(Joystick):
    ...
    def read_data(self) -> ShanWanGamepadInput:
        ...
        # 트리거(아날로그 축)를 디지털 버튼으로 변환 — 절반 이상 당기면 눌림
        elif axis_name == 'L2_TRIGGER':
            self.gamepad_input.button_L2 = 1 if axis_val > 0.0 else 0
        elif axis_name == 'R2_TRIGGER':
            self.gamepad_input.button_R2 = 1 if axis_val > 0.0 else 0
